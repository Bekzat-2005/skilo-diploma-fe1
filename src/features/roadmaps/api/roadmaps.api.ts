...
export const roadmapsApi = {
  ...
  async removeUserRoadmap(roadmapId: string) {
    const { data } = await axiosInstance.delete(`/roadmaps/collection/${roadmapId}`);
    return data;
  },

  async generateAiRoadmap(formData: any) {
    const { data } = await axiosInstance.post('/roadmaps/generate', formData)
    return data
  },

  async getMyAiRoadmaps() {
